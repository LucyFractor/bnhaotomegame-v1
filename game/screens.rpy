...
define base_initialized = False
# $ persistent.saw_credits = False
# $ persistent.saw_trueend_mi = False
# $ persistent.saw_trueend_bk = False
# $ persistent.saw_trueend_ts = False
...
init 5:
    ...
    style phone_message_frame:
        background Solid("#957DAD")
        ypadding 15
        xpadding 15

    style phone_message_frame2:
        background Solid("#D291BC")
        ypadding 15
        xpadding 15
...
screen phone_reply1(reply1, label1):
    modal True
    vbox:
        xalign 0.5
        yalign 0.8
...
screen phone_reply2(reply1, label1, reply2, label2):
    modal True
    vbox:
        xalign 0.5
        yalign 0.8
...
screen main_menu():
    ...
    if not persistent.saw_trueend_mi:
        add "images/background/game_menu_mask_mi.png"
    if not persistent.saw_trueend_ts:
        add "images/background/game_menu_mask_ts.png"
    if not persistent.saw_trueend_bk:
        add "images/background/game_menu_mask_bk.png"

    add "images/background/game_menu_bnhalogo.png"

    if persistent.saw_trueend_mi and persistent.saw_trueend_ts and persistent.saw_trueend_bk:
        hbox:
            imagebutton:
                idle "images/background/all_clear.png"
                hover LiveComposite(
                    (500, 96),
                    (0, 0), "images/background/all_clear.png",
                    (110, 18), Text("All routes completed!\nThanks for playing:)\nClick to drop comment!", size=20, style="myoutline")
                    )
                action OpenURL("https://bnha-scenarios.tumblr.com/ask")
                xpos 430 ypos 980
            null width 50
...
## Characters screen ##############################################################
##
## This is a screen that displays the character descriptions and relationship points
screen characters():
    ...
    use game_menu(_("Characters"), scroll="viewport"):
        ...
        vbox:
            ...
            if base_initialized == True:
                ...
                text _("[char_mc.fullName] (The Player)")
                text _("It's you! :)")
                text _("\n")

                $ char_mi = midoriya.get_self()
                text _("[char_mi.fullName]")
                text _("Affection: [char_mi.affection] / 10")
                text _("\n")

                $ char_bk = bakugou.get_self()
                text _("[char_bk.fullName]")
                text _("Affection: [char_bk.affection] / 10")
                text _("\n")

                $ char_ts = todoroki.get_self()
                text _("[char_ts.fullName]")
                text _("Affection: [char_ts.affection] / 10")
            else:
                text _("Nothing to see here\n")

## This is redefined in options.rpy to add text to the about screen.
define gui.characters = ""
